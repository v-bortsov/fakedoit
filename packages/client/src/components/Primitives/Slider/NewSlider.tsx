...
const normalizeValue = (
  ...
  const getBetweenValue = (inputValue: number) => Math.max(
    Math.min(
      inputValue,
      maximumValue
    ),
...
const updateValues = ({
  ...
  if (Array.isArray(values) && Array.isArray(newValues)) {
    return values?.map((
      value: number | Animated.Value, index: number
    ) => {
...
export class NewSlider extends PureComponent<SliderProps, SliderState> {
  ...
  static getDerivedStateFromProps(
    props: SliderProps, state: SliderState
  ) {
    if (
      props.trackMarks &&
      ...
      const newTrackMarkValues = normalizeValue(
        props,
        props.trackMarks
      );
  ...
  _handleStartShouldSetPanResponder = (e: any,): /* gestureState: GestureState */
  ...
  _setCurrentValueAnimated = (
    value: number, thumbIndex = 0
  ) => {
  ...
  _thumbHitTest = (e: {nativeEvent: any}) => {
    ...
    if (trackClickable) {
      ...
      if (values.length === 1) {
        ...
        const thumbDistances = values.map((
          _value, index
        ) => {
        ...
        >
          ...
          >
            {!!debugTouchArea &&
                interpolatedThumbValues.map((
                  value, i
                ) => this._renderDebugThumbTouchRect(
                  value,
                  i
                ))}
          </View>

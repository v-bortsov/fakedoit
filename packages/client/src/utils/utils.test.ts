...
it("enumToObject", () => {
  expect(enumToObject(DaysOfWeek)).toEqual({
    Sun: 0,
    Mon: 1,
    Tue: 2,
    Wed: 3,
...
it("propFilterAndPluck", () => {
  expect(
    propFilterAndPluck("name", "city", "collect")(initialState.columns)
  ).toEqual([
...
it("cartesianCondition", () => {
  expect(
    cartesianCondition(
      [
        { name: "x", collect: ["a", "b", "c"] },
        { name: "y", collect: ["d", "e"] },
        { name: "z", collect: ["f", "g"] },
      ],
      null
...
it("addParam", () => {
  expect(
    addParam("name", add, [always(1), always(1)])({ pizza: "formadgo" })
  ).toEqual({
    pizza: "formadgo",
    name: 2,,
  });
